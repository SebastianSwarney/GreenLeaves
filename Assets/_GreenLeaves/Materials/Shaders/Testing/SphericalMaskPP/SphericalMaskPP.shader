Shader "Custom/GreenLeaves/SphericalMaskPP"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 uv : TEXCOORD0;
				//pass camera's world-space direction from vertex to the fragment shader
            ...
            v2f vert (appdata v)
            {
                ...
                o.uv = v.uv;
				//calculate the world direction; after getting the matrix that converts clip position 
				//to world position, it's multiplied with the camera's clip position and subtracted 
				//from its world position to get a vector that represents the camera's direction in world-space
			...
			{
				//Get the depth of the camera, //try using depth = Linear01Depth(depth) instead ;)
