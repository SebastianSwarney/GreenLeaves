Shader "GreenLeaves_Custom/SphericalMaskDissolve" {
	Properties{
		...
		SubShader{
			...
			//Cull off to disable directive, so that the backfaces of an object 
			//can be seen if not completely dissolved
			Cull off
			LOD 200
			...
			//parameters associated with the spherical mask, controlled from an outside source
			// _GLOBAL prefix is not necessary but used to indicate that its a property controlled globally
			...
			void surf(Input IN, inout SurfaceOutputStandard o) {
				...
				//clip: if value is smaller than 0, discard the pixel
				clip(sphere - 0.1);
