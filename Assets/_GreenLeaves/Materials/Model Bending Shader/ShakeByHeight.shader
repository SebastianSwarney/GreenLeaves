...
Shader "Custom/ShakeByHeight"
{
	Properties
	{
		...
		_Amount("Amount", Float) = .5
			...
			///These need to be passed over
		[Header(HeightProperties)]
		_Height("Object Height", float) = 0.0
		_XPos("X Pos", float) = 0.0
		_YPos("Y Pos", float) = 0.0
		...
		SubShader
	{
		...
		void vert(inout appdata_full v)
		{
			...
			float percentOfHeight = (o.worldPos.y - (_YPos)) / (_Height - _YPos);
			v.vertex.x += percentOfHeight * (sin((_Time.y * _SpeedX + v.vertex.y * _Amplitude) - _XPos) * _Distance * _Amount) + (pow(percentOfHeight * _XWindAmplitude,2)) * _XWindDir;
			v.vertex.z += percentOfHeight * (cos((_Time.y * _SpeedZ + v.vertex.y * _Amplitude) - _ZPos) * _Distance * _Amount) + (pow(percentOfHeight * _ZWindAmplitude,2)) * _ZWindDir;
		}
